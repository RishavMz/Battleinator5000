...
const head_img = texture.load(
  "https://raw.githubusercontent.com/RishavMz/3D_Battle_Arena/main/client/resources/head.png"
);
const limb_img = texture.load(
  "https://raw.githubusercontent.com/RishavMz/3D_Battle_Arena/main/client/resources/limb.png"
);
const body_img = texture.load(
  "https://raw.githubusercontent.com/RishavMz/3D_Battle_Arena/main/client/resources/body.png"
);
const metal_img = texture.load(
  "https://raw.githubusercontent.com/RishavMz/3D_Battle_Arena/main/client/resources/metal.png"
);
...
export class Point {
  ...
  draw() {
    const head = new THREE.Mesh(
      new THREE.SphereGeometry(1.5, this.texture, this.texture),
      new THREE.MeshBasicMaterial({ map: head_img })
    );
    const body = new THREE.Mesh(
      new THREE.CylinderGeometry(2, 1, 5, this.texture, this.texture, false),
      new THREE.MeshBasicMaterial({ map: body_img })
    );
    const neck = new THREE.Mesh(
      new THREE.SphereGeometry(2, this.texture, this.texture, 10, 10, 0, 1),
      new THREE.MeshBasicMaterial({ map: head_img })
    );
    const hand11 = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ map: limb_img })
    );
    const hand12 = new THREE.Mesh(
      new THREE.CylinderGeometry(0, 0.3, 5, this.texture, this.texture, false),
      new THREE.MeshBasicMaterial({ map: metal_img })
    );
    const hand21 = new THREE.Mesh(
      new THREE.CylinderGeometry(
        0.5,
        0.3,
        4,
    ...
    const hand22 = new THREE.Mesh(
      new THREE.CylinderGeometry(0, 0.3, 5, this.texture, this.texture, false),
      new THREE.MeshBasicMaterial({ map: metal_img })
    );
    const skirt = new THREE.Mesh(
      new THREE.CylinderGeometry(1, 1.8, 3, this.texture, this.texture, false),
      new THREE.MeshBasicMaterial({ map: metal_img })
    );
...
export class QuadTree {
  ...
  contains(point) {
    if (
      point.posx > this.posx - this.side / 2 &&
      point.posx < this.posx + this.side / 2 &&
      point.posz > this.posz - this.side / 2 &&
      point.posz < this.posz + this.side / 2
    ) {
      return this;
    }
  ...
  squareContains(posx, posz, side, point) {
    ...
    if (
      ...
      if (this.northeast) {
        this.northeast.squareContains(posx, posz, side, pointdata);
      }
      if (this.northwest) {
        this.northwest.squareContains(posx, posz, side, pointdata);
      }
      if (this.southeast) {
        this.southeast.squareContains(posx, posz, side, pointdata);
      }
      if (this.southwest) {
        this.southwest.squareContains(posx, posz, side, pointdata);
      }
    }
    return pointdata;
  }
  draw() {
    let box = new THREE.Mesh(
      new THREE.BoxGeometry(this.side, this.side / 10, this.side),
      ...
      if (this.points.length <= this.limit) {
        ...
        if (this.northwest === undefined) {
          this.northwest = new QuadTree(
            this.scene,
            this.posx - this.side / 4,
            this.posz - this.side / 4,
            this.side / 2
          );
