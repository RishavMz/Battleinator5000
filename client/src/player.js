...
const head_img = texture.load(
  "https://raw.githubusercontent.com/RishavMz/3D_Battle_Arena/main/client/resources/player_head.png"
);
const body_img = texture.load(
  "https://raw.githubusercontent.com/RishavMz/3D_Battle_Arena/main/client/resources/player_body.png"
);
const limb_img = texture.load(
  "https://raw.githubusercontent.com/RishavMz/3D_Battle_Arena/main/client/resources/player_limb.png"
);
const metal_img = texture.load(
  "https://raw.githubusercontent.com/RishavMz/3D_Battle_Arena/main/client/resources/player_metal.png"
);
...
class Sword {
  constructor() {
    this.tool = new THREE.Mesh(
      new THREE.CylinderGeometry(0.5, 0, 10, 100),
...
class Axe {
  constructor() {
    const handle = new THREE.Mesh(
      new THREE.CylinderGeometry(0.2, 0.2, 10, 100),
...
class Polearm {
  constructor() {
    this.tool = new THREE.Mesh(
      new THREE.CylinderGeometry(0.5, 0.2, 20, 100),
      new THREE.MeshBasicMaterial({ map: bark_image })
    );
    this.range = 20;
    this.holder = 8;
...
export class Player {
  constructor(scene, quadtree, chunk, username, posx, posz, texture) {
    ...
    const head = new THREE.Mesh(
      new THREE.SphereBufferGeometry(1.5, this.texture, this.texture),
    ...
    const waistGuard = new THREE.Mesh(
      new THREE.TorusBufferGeometry(
        0.8,
        0.8,
        this.texture,
    ...
    const headGuard = new THREE.Mesh(
      new THREE.TorusBufferGeometry(
        0.8,
        0.8,
        this.texture,
    ...
    const font = floader.load(
      "https://raw.githubusercontent.com/RishavMz/3D_Battle_Arena/main/client/resources/font.json",
      (font) => {
        ...
        this.playername = new THREE.Mesh(tgeometry, [
          new THREE.MeshDepthMaterial({ color: 0xffffff }),
          new THREE.MeshDepthMaterial({ color: 0x000000 }),
        ]);
    ...
    this.playerrange = new THREE.Mesh(
      new THREE.BoxBufferGeometry(this.range, 10, this.range, 5, 5, 5),
      new THREE.MeshBasicMaterial({ color: 0x7bff00, wireframe: true })
    );
  }
  draw() {
    this.player.position.x = this.posx;
